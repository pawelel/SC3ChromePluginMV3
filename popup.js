...
const reportLabel = (document.querySelector("#report-label").innerHTML =
  chrome.i18n.getMessage("reportlabel"));
const outageLabel = (document.querySelector("#outage-label").innerHTML =
  chrome.i18n.getMessage("outagelabel"));
const clearFilter = (document.querySelector("#filter-clear").innerHTML =
  chrome.i18n.getMessage("clearfilter"));
...
const announcementsLabel = document.getElementById("announcements-label");
announcementsLabel.innerHTML = chrome.i18n.getMessage("announcements");
const assetsLabel = document.getElementById("assets-label");
assetsLabel.innerHTML = chrome.i18n.getMessage("assets");
...
chrome.runtime.sendMessage({ name: "fetchAssets" }, (response) => {
    ...
    columns: [
      {
        ...
        field: "asset",
        headerFilter: "input",
        headerFilterPlaceholder: "assets",
        tooltip: function (cell) {
          //cell - cell component
          inerText = "";
          var assetRow = cell.getData();
          var assetName = assetRow.asset;
          var situations = filterByValue(response, assetName)[0].situations;
          for (const situation of situations) {
            inerText += situation.name + "\n";
          }
      ...
      },
      {
        ...
        field: "place",
        headerFilter: "input",
        headerFilterPlaceholder: "places",
        width: 100,
      },
      {
        title: "Device",
        field: "device",
        headerFilter: "input",
        headerFilterPlaceholder: "devices",
        width: 100,
      },
      {
        title: "Model",
        field: "model",
        headerFilter: "input",
        headerFilterPlaceholder: "models",
        width: 100,
      },
      {
        title: "Coordinate",
        field: "coordinate",
        headerFilter: "input",
        headerFilterPlaceholder: "coordinates",
        width: 170,
      },
    ],
  });
  ...
  table.on("rowClick", function (e, row) {
    ...
    generateOptions(situationList).forEach((option) =>
      situationSelect.appendChild(option)
    );
    setSituation(situationList, assetRow);
    situationSelect.addEventListener("change", (changeEvent) =>
      setSituation(situationList, assetRow)
    );
  });
});
...
document.getElementById("download-xlsx").addEventListener("click", function () {
  table.download("xlsx", "AssetList" + assetDate + ".xlsx", {
    sheetName: "My Data",
  });
});
...
function setDate() {
  if (reportDate.value === "") {
    ...
    reportDate.addEventListener("change", (changeEvent) => {
      reportTime =
        "Report time: " + changeEvent.target.value.replace("T", " ") + "\n";
      outputDescription.value = reportTime + outageTime + generalDescription;
    });
  }
}
...
function setSituation(situationList, assetRow) {
  ...
  if (
    situationName &&
    situationName != chrome.i18n.getMessage("seconddropdowntext") &&
    situationName != chrome.i18n.getMessage("firstdropdowntext") &&
    assetRow.asset
  ) {
    ...
    if (situationSelect?.options[situationSelect?.selectedIndex]?.value > 0) {
      generalDescription = situationList
        .find((situation) => situation.name === situationName)
        .description.join("\n");
    }
    outputDescription.value = reportTime + outageTime + generalDescription;
  } else {
